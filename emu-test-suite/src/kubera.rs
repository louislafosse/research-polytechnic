...
"#;
    ...
    // Run the test
    let output = Command::new(temp_bin).output();
    ...
    let status_str = stdout
        .lines()
        .filter(|line| {
            !line.is_empty() && !line.contains("KUBERA") && !line.contains("Access violation")
        })
        .last()
        .unwrap_or("0")
        .trim();

    let fpu_status = u16::from_str_radix(status_str, 16).unwrap_or(0);
    ...
    let has_unsupported =
        stdout.contains("Unsupported instruction") || stderr.contains("Unsupported instruction");
...
int main() {{
    kubera::KUBERA ctx{{}};
    ctx.decoder->reconfigure(shellcode, sizeof(shellcode), 0);
...
"#,
    ...
    let temp_cpp = format!("/tmp/kubera_{}_test.cpp", test_name);
    let temp_bin = format!("/tmp/kubera_{}_test", test_name);
    ...
    if !compile.status.success() {
        ...
        return Err(format!("KUBERA compilation failed: {}", stderr).into());
    }
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if stdout.contains("Unsupported instruction") || stderr.contains("Unsupported instruction") {
        return Err("KUBERA unsupported instruction".into());
    }
    ...
    Ok(u64::from_str_radix(status_str, 16).unwrap_or(0))
}
