...
fn panic_payload_to_string(panic_info: Box<dyn std::any::Any + Send>) -> String {
...
fn friendly_probe_name(probe_name: &str) -> &str {
    match probe_name {
        "fpu_overflow" => "FPU stack overflow",
        "lahf_flags" => "LAHF flags",
        "rdtscp_aux" => "RDTSCP AUX",
        "tsc_monotonic" => "TSC monotonicity",
        "x87_empty_fstp" => "x87 empty FSTP",
        "mxcsr_round_down" => "MXCSR round-down",
        "sldt" => "SLDT",
        "str" => "STR",
        "smsw" => "SMSW",
        _ => probe_name,
...
fn classify_probe_panic(probe_name: &str, panic_msg: &str) -> String {
    if panic_msg.contains("FPU stack overflow") {
        return "MWEMU detected FPU overflow but panicked instead of setting flags (IE:? SF:? C1:?)"
            .to_string();
    }

    if panic_msg.contains("Option::unwrap()") || panic_msg.contains("None value") {
        return format!(
            "MWEMU internal bug while running {}: emulator panicked on an unexpected missing value.",
            friendly_probe_name(probe_name)
        );
    }

    if panic_msg.contains("not implemented") || panic_msg.contains("Unsupported") {
        return format!(
            "MWEMU does not support the {} probe cleanly.",
            friendly_probe_name(probe_name)
        );
    }

    format!(
        "MWEMU failed while running {}: {}",
        friendly_probe_name(probe_name),
        panic_msg
    )
}
...
pub fn test_fpu_in_mwemu() -> Result<(u16, String), Box<dyn std::error::Error>> {
    ...
    let result = panic::catch_unwind(|| -> Result<(u16, String), Box<dyn std::error::Error>> {
        ...
        let _result_map = emu.alloc(
            "result",
            result_size,
            libmwemu::maps::mem64::Permission::READ_WRITE,
        );
...
pub fn run_shellcode_return_rax(
    probe_name: &str,
    shellcode: &[u8],
    ...
    let result = panic::catch_unwind(|| -> Result<u64, Box<dyn std::error::Error>> {
        let mut emu = libmwemu::emu64();
        emu.init_linux64(false);
        emu.load_code_bytes(shellcode);

        match emu.run(None) {
            Ok(_) => Ok(emu.regs().rax),
            Err(e) => Err(format!("MWEMU execution failed: {:?}", e).into()),
        }
    });
    ...
    match result {
        ...
        Err(panic_info) => {
            let panic_msg = panic_payload_to_string(panic_info);
            Err(classify_probe_panic(probe_name, &panic_msg).into())
        }
    }
}
