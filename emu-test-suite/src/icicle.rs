...
pub fn test_fpu_in_icicle() -> Result<(u16, String), Box<dyn std::error::Error>> {
    ...
    let vm = match icicle_vm::build_with_path(
        &cpu_config,
        std::path::Path::new("./data/Ghidra/Processors"),
    ) {
        Ok(v) => v,
        Err(e) => {
    ...
    if !vm
        .cpu
        .mem
        .map_memory_len(code_addr, code_size as u64, code_mapping)
    {
    ...
    vm.cpu
        .mem
        .write_bytes(code_addr, &shellcode, perm::NONE)
    ...
    if !vm
        .cpu
        .mem
        .map_memory_len(result_addr, result_size, result_mapping)
    {
    ...
    if !vm
        .cpu
        .mem
        .map_memory_len(stack_addr, stack_size, stack_mapping)
    {
    ...
    vm.cpu
        .mem
        .write_bytes(halt_addr, &[0xf4], perm::NONE)
        .map_err(|e| format!("Failed to write halt instruction: {:?}", e))?;

    // Write return address to stack
    vm.cpu
        .mem
        .write_bytes(stack_top, &halt_addr.to_le_bytes(), perm::NONE)
    ...
    let rip_var = vm
        ...
        .sleigh
        .get_reg("RIP")
        .ok_or("RIP register not found")?;
    let rsp_var = vm
        .cpu
        .arch
        .sleigh
        .get_reg("RSP")
        .ok_or("RSP register not found")?;
    ...
    vm.cpu
        .mem
        .read_bytes(result_addr, &mut result_bytes, perm::NONE)
...
pub fn run_shellcode_return_rax(shellcode: &[u8]) -> Result<u64, Box<dyn std::error::Error>> {
    ...
    let mut vm = icicle_vm::build_with_path(
        &cpu_config,
        std::path::Path::new("./data/Ghidra/Processors"),
    )
    .map_err(|e| {
        let err_str = e.to_string();
        if err_str.contains("Sleigh spec not found") || err_str.contains("sleigh") {
            "Icicle test skipped (missing Ghidra processor specs)".into()
        } else {
            Box::<dyn std::error::Error>::from(e)
        }
    })?;

    let capacity = 8 * 2 * 50_000;
    vm.cpu.mem.set_capacity(capacity);

    let code_addr = 0x10000u64;
    let code_size = ((shellcode.len() + 0xfff) / 0x1000) * 0x1000;
    let stack_addr = 0x200000u64;
    ...
    let exec_mapping = icicle_vm::cpu::mem::Mapping {
        perm: perm::EXEC | perm::READ | MAPPING_PERMISSIONS,
        value: 0,
    };
    let rw_mapping = icicle_vm::cpu::mem::Mapping {
        perm: perm::READ | perm::WRITE | MAPPING_PERMISSIONS,
        value: 0,
    };
    ...
    if !vm
        .cpu
        .mem
        .map_memory_len(code_addr, code_size as u64, exec_mapping)
    {
        return Err("Failed to map code memory".into());
    }
    vm.cpu
    ...
    if !vm
        .cpu
        .mem
        .map_memory_len(stack_addr, stack_size, rw_mapping)
    {
        return Err("Failed to map stack memory".into());
    }
    vm.cpu
    ...
    if !vm.cpu.mem.map_memory_len(halt_addr, 0x1000, exec_mapping) {
        return Err("Failed to map halt memory".into());
    }
    vm.cpu
        .mem
    ...
    let rsp = vm
        ...
        .ok_or("RSP register not found")?
        .get_var()
        .ok_or("RSP variable missing")?;
    let rax = vm
        .cpu
        .arch
        .sleigh
        .get_reg("RAX")
        .ok_or("RAX register not found")?
        .get_var()
        .ok_or("RAX variable missing")?;

    vm.cpu.write_reg(rip, code_addr);
    vm.cpu.write_reg(rsp, stack_top);
    vm.cpu.write_reg(rax, 0);
    vm.icount_limit = vm.cpu.icount.saturating_add(10000);

    match vm.run() {
        icicle_vm::VmExit::InstructionLimit => {}
        icicle_vm::VmExit::Halt => {}
        icicle_vm::VmExit::UnhandledException(code) => {
            return Err(format!("Icicle unhandled exception: {:?}", code).into());
        }
        icicle_vm::VmExit::Unimplemented => {
            return Err("Icicle unimplemented instruction".into());
        }
        other => {
            return Err(format!("Icicle execution failed: {:?}", other).into());
        }
    }

    Ok(vm.cpu.read_reg(rax))
}
